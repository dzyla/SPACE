...
def main():
    ...
    - **Multiple Sequence Alignment (MSA):** Perform alignments using **Clustal Omega**, visualizing the results in an interactive plot.
    ...
    ## How to cite
    - Zyla, D. **SPACE**: Sequence Protein Alignment and Conservation Engine. Streamlit app. [GitHub]
    ...
    with st.sidebar.form("parameters_form"):
        ...
        use_refseq = st.checkbox("Use RefSeq database", value=False)
        ...
        clustalo_default = (
            "E:\\Downloads\\programy\\clustal-omega-1.2.2-win64\\clustalo.exe"
            if os.path.exists(
                r"E:\Downloads\programy\clustal-omega-1.2.2-win64\clustalo.exe"
            )
            else "clustalo"
        )
        ...
        al2co_default = (
            r"E:\Downloads\programy\clustal-omega-1.2.2-win64\al2co.exe"
            if os.path.exists(
                r"E:\Downloads\programy\clustal-omega-1.2.2-win64\al2co.exe"
            )
            else "al2co"
        ...
        if not email:
            ...
            try:
                ...
                if clustalo_path_resolved and al2co_path_resolved:
                    ...
                    with st.spinner("Fetching sequences..."):
                        if data_source == "NCBI":
                            file_path = search_and_save_protein_ncbi(
                                query, filename, email, max_seqs=max_seqs, use_refseq=use_refseq
                            )
                        else:
                    ...
                    if file_path:
                        proteins_list = get_protein_sequences(file_path)
                        ...
                        st.session_state.al2co_output = None
                        st.session_state.al2co_df = None
                        st.session_state.alignment_mapping = None  # Reset alignment mapping
                        st.session_state.unique_mutations = {}
                        st.session_state.excluded_sequences = {}
                        st.session_state.mutation_summary = set()
                        st.session_state.all_mutations_str = ""
    ...
    if st.session_state.get("proteins_list") and not st.session_state.get("msa_done"):
        ...
        ref_seq_choice = st.radio("Select reference sequence:", ["Select from list", "Use provided sequence"], horizontal=True)
        ...
        if ref_seq_choice == "Select from list":
            ...
            sorted_protein_list = sorted(st.session_state.proteins_list, key=lambda seq: seq.id)

            # Now create the seq_options from the sorted list
            seq_options = [
                f"{seq.id} (Length: {len(seq.seq)})"
                for seq in sorted_protein_list
            ]
            
            # You can reverse the sorted list if needed
            seq_options = seq_options[::-1]  # Reverse the sorted seq_options
            sorted_protein_list = sorted_protein_list[::-1]  # Reverse the sorted proteins list as well
            ...
            st.session_state.reference_seq = clean_fasta(st.text_area(
    ...
    if not st.session_state.get("msa_done"):
        ...
        uploaded_msa = st.file_uploader("Upload MSA File (FASTA or Clustal format):", type=["fasta", "fa", "aln", "clustal"])
        if uploaded_msa:
            try:
    ...
    if st.session_state.get("result") and not st.session_state.get("msa_done"):
        ...
        with st.form("Adjust Filtering Parameters"):
            ...
            all_seq_lengths = [len(seq) for seq in st.session_state.result[1]] if st.session_state.result else []
            ...
            st.write('Filtering sequences based on sequence length and alignment score to the reference sequence.')
    ...
    if st.session_state.get("unique_mutations"):
        ...
        st.subheader("Concatenated String of All Unique Point Mutations")
        ...
        if st.session_state.get("phylogenetic_tree") is None:
            if not len(st.session_state.result[0]) > 200:
                ...
                if tree_file:
                    st.session_state.phylogenetic_tree = tree_file  # Store the tree file path
                    plot_phylogenetic_tree(tree_file, c2)
                else:
                    c2.warning("Phylogenetic tree could not be generated.")
            else:
                c2.warning(f"Phylogenetic tree generation is disabled for more than 200 sequences. Number of sequences: {len(st.session_state.result[0])}")
                if c2.checkbox('Generate tree with random sample of 200 sequences'):
                    tree_file = generate_phylogenetic_tree(st.session_state.msa_outfile, use_random_subsample=True)
                    if tree_file:
    ...
    if st.session_state.get("msa_done") and st.session_state.get("al2co_output"):
        pdb_server = st.radio("Select PDB source:", ["PDB", "AlphaFold", "Upload Your Own PDB"], horizontal=True)
        ...
        if pdb_server == "PDB":
            ...
            uniprot_ids = extract_uniprot_ids(mutations_df['Sequence ID'].to_list(), include_version=False)
            if uniprot_ids:
                if len(uniprot_ids) > 1:
                    ...
                    for uniprot_id_item in uniprot_ids:
                        ...
                        if uniprot_meta_data:
                            uniprot_name = uniprot_meta_data.get('proteinDescription', {}).get('recommendedName', {}).get('fullName').get('value')
                            uniprot_kbId = uniprot_meta_data.get('uniProtkbId')
                            uniprot_names.append(f'{uniprot_kbId} - {uniprot_name}')
                    selected_uniprot_id = uniprot_ids[uniprot_names.index(st.selectbox("Select UniProt ID:", uniprot_names))]
            ...
            st.session_state.user_pdb = st.file_uploader("Upload Your Own PDB File:", type=["pdb", "ent"])
            if st.session_state.user_pdb:
                ...
                sequence_score = None  # Prevent further processing until PDB is uploaded
        ...
        try:
            ...
            if specify_chain:
                ...
                selected_chain = max(
                    result['chain_data'], key=lambda x: result['chain_data'][x]['alignment_score']
                )
                c1.write(
                    f"**Best Matching Chain:** `{selected_chain}` with Alignment Score: `{result['chain_data'][selected_chain]['alignment_score']}`"
                )
            if result['chain_data'][selected_chain]['alignment_score'] < len(st.session_state.reference_seq) * 0.2:
            ...
            for residue in annotation_residues:
                ...
                res_num = int(res_num)
                color = gradient_dict.get(score, "#deddda")  # Default color if not found
                if annotation:
                    xyzview.addLabel(
                        str(res_num),
                        {
    ...
    if st.session_state.get("msa_done"):
        ...
        with open(f"{clean_string_for_path(query)}.txt", "w") as f:
            ...
            f.write(f"Job ran at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
